...
景仰而望之怯步。故效先贤之斧凿，择至简子集以叙数据。虽为小器，举轻若重，尚其中
...
注三：wson 文档建议用 `.wson` 作为文件后缀名，或用 `.文语`。但它本质上只是纯文配
件，与后缀无关，但须用 utf-8 编码。
...
```
表之『诗人』曰『李白』之『生年』曰七零一曰之『卒年』七六二之『在世』曰阴也
```
...
* 字符串的双引号，有繁体『』与简体“”两对。表内的名字也须用双引号括起。转义是程序
  ...
  号与换行符、制表符。一般用户用到的机率小，且以『』为引号时，内部的“”的不必转
  义，反之亦然。
